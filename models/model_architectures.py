...
class EmptyBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.layer_dict['Identity'](out)
...
class EntryConvolutionalBlock(nn.Module):
    ...
    def build_module(self):
        ...
        out = F.leaky_relu(self.layer_dict['bn_0'](out))
...
class ConvolutionalProcessingBlock(nn.Module):
    "basic cascade of 2 convolutional layers, each followed by a Leaky ReLU activation function"
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
...
class ConvolutionalDimensionalityReductionBlock(nn.Module):
    "basic cascade of 2 convolutional layers, with an average pooling layer in the middle"
...
class ConvolutionalNetwork(nn.Module):
    ...
    def build_module(self):
        ...
        self.layer_dict['input_conv'] = EntryConvolutionalBlock(input_shape=out.shape, num_filters=self.num_filters,
                                                                ...
                                                                dilation=1)
        out = self.layer_dict['input_conv'](out)
        ...
        for i in range(self.num_stages):  # for number of layers times
            for j in range(self.num_blocks_per_stage):
                self.layer_dict['block_{}_{}'.format(i, j)] = self.processing_block_type(input_shape=out.shape,
                                                                                         ...
                                                                                         padding=1)
                out = self.layer_dict['block_{}_{}'.format(i, j)](out)
            self.layer_dict['reduction_block_{}'.format(i)] = self.dimensionality_reduction_block_type(
                ...
                padding=1,
                reduction_factor=2)
            out = self.layer_dict['reduction_block_{}'.format(i)](out)
...
class ConvolutionalProcessingBlockBatchNormalized(nn.Module):
    "basic cascade of 2 convolutional layers, each followed by a BN and a Leaky ReLU activation function"
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalProcessingBlockBatchNormalized, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict['conv_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_0'] = nn.BatchNorm2d(num_features=self.num_filters)  # Batch Normalization # 0

        out = self.layer_dict['conv_0'](out)
        out = self.layer_dict['bn_0'](out)  # Batch Normalization after convolution and before activation
        out = F.leaky_relu(out)

        self.layer_dict['conv_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_1'] = nn.BatchNorm2d(num_features=self.num_filters)  # Batch Normalization # 1

        out = self.layer_dict['conv_1'](out)
        out = self.layer_dict['bn_1'](out) # Batch Normalization after convolution and before activation
        out = F.leaky_relu(out)
...
class ConvolutionalProcessingBlockBatchNormalizedResConn(nn.Module):
    "basic cascade of 2 convolutional layers, each followed by a BN and a Leaky ReLU activation function"
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalProcessingBlockBatchNormalizedResConn, self).__init__()
    ...
    def build_module(self):
        ...
        # self.mismatch_proj = nn.Conv2d(in_channels=x.shape[1], out_channels=self.num_filters, bias=False,
        #                                 kernel_size=1, dilation=self.dilation,
        #                                 padding=0, stride=1)
        self.layer_dict['conv_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=out.shape[1], bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_0'] = nn.BatchNorm2d(num_features=out.shape[1])  # Batch Normalization # 0

        # res = self.mismatch_proj(x) # Skip connection
        res = x
        ...
        self.layer_dict['conv_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=out.shape[1], bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_1'] = nn.BatchNorm2d(num_features=out.shape[1])  # Batch Normalization # 1
...
class ConvolutionalDimensionalityReductionBlockBatchNormalized(nn.Module):
    "basic cascade of 2 convolutional layers, each followed by a BN , with an average pooling layer in the middle"
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ConvolutionalDimensionalityReductionBlockBatchNormalized, self).__init__()
